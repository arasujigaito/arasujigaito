...
export default function FollowersPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        // users/{targetUid}/followers のドキュメントIDが「相手uid」前提
  ...
  if (errorText) {
    return (
      <div style={pageBg}>
        <div style={{ color: "white", padding: 20, maxWidth: 960, margin: "0 auto" }}>
          <div style={{ fontWeight: 800, marginBottom: 8 }}>フォロワー一覧でエラー</div>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "16px", color: "white" }}>
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ fontSize: 18, fontWeight: 800 }}>フォロワー</div>
            <button
              type="button"
              onClick={() => router.back()}
              style={{
        ...
        {rows.length === 0 ? (
          <div style={{ opacity: 0.8 }}>まだフォロワーはいません</div>
        ) : (
